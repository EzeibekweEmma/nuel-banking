...
export function AuthForm({ mode }: { mode: 'login' | 'register' }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      const tokens = mode === 'login'
        ...
        : await api.register({ email: String(form.get('email')), firstName: String(form.get('firstName')), lastName: String(form.get('lastName')), password });
      saveTokens(tokens);
      window.location.assign('/dashboard');
    } catch (reason) {
      setError(reason instanceof ApiError ? reason.message : 'Unable to continue right now.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <form onSubmit={submit} className="mt-7 space-y-4">
      ...
      {mode === 'register' && <p className="text-[11px] leading-5 text-[#7b8b87]">Use at least 12 characters. A mix of words, numbers, and symbols works best.</p>}
      ...
      <button disabled={loading} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#087a5b] text-sm font-bold text-white shadow-[0_12px_24px_-16px_rgba(6,105,79,.8)] transition hover:bg-[#06694f] disabled:bg-[#a8bdb6]">
...
function Input({ name, label, type = 'text', autoComplete }: { name: string; label: string; type?: string; autoComplete: string }) {
  return <label className="block text-xs font-bold text-[#39574f]">{label}<input required name={name} type={type} autoComplete={autoComplete} className="mt-2 h-12 w-full rounded-xl border border-[#ccd8d3] bg-[#fbfcfb] px-4 text-sm font-normal text-[#18352e] outline-none transition focus:border-[#087a5b] focus:bg-white" /></label>;
}
