...
export function CustomerShell({ children }: { children: ReactNode }) {
  ...
  const details = pageDetails[pathname] ?? (pathname.startsWith('/transactions/')
    ...
    : { eyebrow: 'Banking', title: 'Astra Bank' });
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[276px] flex-col bg-[#092d24] px-4 py-5 text-white transition-transform duration-300 lg:sticky lg:top-0 lg:h-screen lg:w-auto lg:translate-x-0 ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-2">
          <Link href="/dashboard" className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#d8f85c] text-base font-black text-[#092d24]">A</span>
            <span><span className="block text-[17px] font-bold tracking-tight">Astra Bank</span><span className="block text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8fb0a7]">Personal banking</span></span>
          </Link>
