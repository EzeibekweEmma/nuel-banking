...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen min-h-dvh bg-[#f3f6f4] lg:grid lg:grid-cols-[250px_minmax(0,1fr)]">
      {menuOpen && <button aria-label="Close menu" className="fixed inset-0 z-[60] cursor-default bg-[#10231f]/45 backdrop-blur-sm lg:hidden" onClick={() => setMenuOpen(false)} />}
      <aside className={'fixed inset-y-0 left-0 z-[70] flex w-[min(276px,calc(100vw-24px))] flex-col overflow-y-auto bg-[#102921] px-4 py-5 text-white transition-transform duration-300 lg:sticky lg:top-0 lg:z-50 lg:h-screen lg:w-auto lg:translate-x-0 ' + (menuOpen ? 'translate-x-0' : '-translate-x-full')}>
        <div className="flex items-center justify-between px-2">
          <Link href="/admin" className="flex items-center gap-3"><BrandMark className="h-10 w-10 shrink-0" /><span><span className="block text-[17px] font-bold">Nuel Bank</span><span className="block text-[10px] font-semibold uppercase tracking-[.18em] text-[#d8f85c]">Staff console</span></span></Link>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-30 flex min-h-[68px] items-center border-b border-[#dce5e1] bg-[#f3f6f4]/95 px-3 py-2 backdrop-blur min-[380px]:px-4 sm:min-h-[76px] sm:px-5 lg:px-8">
          ...
          <div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#087a5b]">Administration</p><h1 className="mt-1 truncate text-base font-bold tracking-tight text-[#18352e] sm:text-xl">{titles[pathname] ?? 'Staff console'}</h1></div>
        ...
        <main className="page-enter min-w-0 p-3 min-[380px]:p-4 sm:p-5 lg:p-8">{children}</main>
      </div>
    </div>
  );
}
