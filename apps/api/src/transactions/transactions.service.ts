  ...
  async transfer(senderId: string, dto: CreateTransferDto) {
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        const transaction = await tx.transaction.create({ data: { sourceAccountId: sender.id, destinationAccountId: recipient.id, amount, reference: randomUUID(), description: dto.description, status: TransactionStatus.COMPLETED, completedAt: new Date() } });
