...
export class ChatbotService {
  ...
  private async generateResponse(
    ...
    message: string,
    context: string,
    history: Array<{ content: string; isFromCustomer: boolean }>,
  ): Promise<string> {
    try {
      ...
      const recentConversation = history
        .map(
          (item) =>
            (item.isFromCustomer ? "Customer: " : "Assistant: ") + item.content,
        )
        .join("\n")
        .slice(-8_000);
      ...
      return (
        result.output_text?.trim() || this.fallbackResponse(userId, message)
      );
    } catch {
      this.logger.warn(
        "Gemini was unavailable; a safe local banking response was used.",
      );
      return this.fallbackResponse(userId, message);
    }
  }
  ...
  private async fallbackResponse(
    ...
    if (
      /(password|passcode|pin|otp|one[- ]?time code|access token)/.test(lower)
    ) {
    ...
    if (
      /what.*(app|nuel).*about|what can (the |this )?app do|app features|about nuel/.test(
        lower,
      )
    ) {
    ...
    if (/balance|how much.*account|available funds/.test(lower)) {
      ...
      return (
        ...
        this.formatMoney(account.balance.toString(), account.currency) +
        " in your " +
    ...
    if (/recent|transaction|payment history|last transfer/.test(lower)) {
      ...
      if (items.length === 0)
        return "You do not have any account activity yet. New credits and debits will appear in your transaction history.";
      ...
      if (latest.kind === "DEPOSIT")
        return (
          "Your most recent activity was a demo deposit of " +
          amount +
          ". Its status is " +
          latest.status.toLowerCase() +
          "."
        );
      return latest.direction === "CREDIT"
        ? "Your most recent activity was " +
            amount +
            " received from " +
            latest.counterparty.name +
            ". Its status is " +
            latest.status.toLowerCase() +
            "."
        : "Your most recent activity was " +
      ...
      const recent = transactions
        ...
        .map(
          (item) =>
            ...
            item.amount +
            ", risk " +
            this.fraud.describeAssessment(item.fraudAssessment),
        )
        .join("; ");
